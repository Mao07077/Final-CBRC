...
const PreTestPage = () => {
  ...
  const fetchPreTest = async () => {
    try {
      setIsLoading(true);
      const data = await moduleService.getPreTest(moduleId);
      setPreTest(data);
    } catch (error) {
      console.error("Failed to fetch pre-test:", error);
    ...
    try {
      ...
      navigate("/pre-test-results", { 
        state: { 
          ...
          moduleTitle: preTest.title.replace("Pre-Test for ", ""),
          moduleId: moduleId
        }
      });
    } catch (error) {
  ...
  if (!preTest || !preTest.questions) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        <div className="text-gray-600 text-xl mb-4">No pre-test available for this module.</div>
        <button
          onClick={() => navigate("/student/dashboard")}
          className="bg-primary text-white px-6 py-2 rounded-lg hover:bg-primary-dark"
        >
  ...
  const currentQuestionData = preTest.questions[currentQuestion];
  const isLastQuestion = currentQuestion === preTest.questions.length - 1;
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-primary-dark mb-2">{preTest.title}</h1>
            <div className="flex justify-between items-center text-sm text-gray-600">
              <span>Question {currentQuestion + 1} of {preTest.questions.length}</span>
              <div className="w-64 bg-gray-200 rounded-full h-2">
                <div 
                  className="bg-primary h-2 rounded-full transition-all duration-300"
                  style={{ width: `${((currentQuestion + 1) / preTest.questions.length) * 100}%` }}
                ></div>
              </div>
            </div>
          </div>
